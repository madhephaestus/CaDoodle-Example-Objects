...
CSG getObject(){
	if(args==null)
		args=["Test_key_here"]
	StringParameter size = new StringParameter(	"compressionSpring"+" Default","GhostSpring",Vitamins.listVitaminSizes("compressionSpring"))	
	if(size.getStrValue().length()==0)
		size.setStrValue("GhostSpring")
	...
	if(args.size()>1)if(!args[1].get("PreventBomAdd"))
		CaDoodleFile.getBoM().addVitamin(new VitaminLocation(false,	args[0], "compressionSpring" , word.getStrValue(), new TransformNR()),true) 
