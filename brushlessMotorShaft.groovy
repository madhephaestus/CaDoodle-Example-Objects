...
CSG getObject(){
	if(args==null)
		args=["Test_key_here"]
	StringParameter size = new StringParameter(	"brushlessMotorShaft"+" Default","TL2955",Vitamins.listVitaminSizes("brushlessMotorShaft"))	
	if(size.getStrValue().length()==0)
		size.setStrValue("TL2955")
	...
	if(args.size()>1)if(!args[1].get("PreventBomAdd"))
		CaDoodleFile.getBoM().addVitamin(new VitaminLocation(false,	args[0], "brushlessMotorShaft" , word.getStrValue(), new TransformNR()),true) 
