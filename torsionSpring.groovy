...
CSG getObject(){
	if(args==null)
		args=["Test_key_here"]
	StringParameter size = new StringParameter(	"torsionSpring"+" Default","Torsion-dummmy",Vitamins.listVitaminSizes("torsionSpring"))	
	if(size.getStrValue().length()==0)
		size.setStrValue("Torsion-dummmy")
	...
	if(args.size()>1)if(!args[1].get("PreventBomAdd"))
		CaDoodleFile.getBoM().addVitamin(new VitaminLocation(false,	args[0], "torsionSpring" , word.getStrValue(), new TransformNR()),true) 
