...
CSG getObject(){
	...
	LengthParameter x = new LengthParameter(csgdb,	args[0]+"_CaDoodle_CubeGeneration_Y",
			20,options)
	LengthParameter y = new LengthParameter(csgdb,	args[0]+"_CaDoodle_CubeGeneration_X",
			20,options)
	LengthParameter z = new LengthParameter(csgdb,	args[0]+"_CaDoodle_CubeGeneration_Z",
			20,options)
	ArrayList<Object> radOpts = []
	for(int i=0;i<Math.min(Math.min( y.getMM(),y.getMM()),z.getMM())/2;i++) {
		radOpts.add(i)
	}
	LengthParameter rad = new LengthParameter(csgdb,	args[0]+"_CaDoodle_CubeGeneration_Round",
			0,radOpts)
	LengthParameter chamfer = new LengthParameter(csgdb,	args[0]+"_CaDoodle_CubeGeneration_Chamfer",
			0,radOpts)
	LengthParameter sides = new LengthParameter(csgdb,	args[0]+"_CaDoodle_CubeGeneration_Sides",
		8,optionsSides)
	CSG local = null
	try {
		if(rad.getMM()>0.01) {
			...
			local.setParameter(csgdb,x).setParameter(csgdb,y).setParameter(csgdb,z);
			rad.setMM(0);
		}
		if(local==null)
