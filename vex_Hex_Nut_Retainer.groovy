...
CSG getObject(){
	if(args==null)
		args=["Test_key_here"]
	StringParameter size = new StringParameter(	"vex_Hex_Nut_Retainer"+" Default","Flat_Bearing",Vitamins.listVitaminSizes("vex_Hex_Nut_Retainer"))	
	if(size.getStrValue().length()==0)
		size.setStrValue("Flat_Bearing")
	...
	if(args.size()>1)if(!args[1].get("PreventBomAdd"))
		CaDoodleFile.getBoM().addVitamin(new VitaminLocation(false,	args[0], "vex_Hex_Nut_Retainer" , word.getStrValue(), new TransformNR()),true) 
